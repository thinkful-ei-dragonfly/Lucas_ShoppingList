...
function renderShoppingList(searchEntry) {
  ...
  if (STORE.items.find(item => item.name === searchEntry)) {
    filteredItems = filteredItems.filter(item => item.name === searchEntry);
  }
...
function updateNameforListItem(itemId, newName) {
  // console.log("Toggling checked property for item with id " + itemId);
  const item = STORE.items.find(item => item.id === itemId);
  item.name = newName;
}
function handleSearchFilter(){
  $('#js-shopping-list-search-form').submit(function(event) {
    event.preventDefault();
    const searchEntry = $('.js-shopping-list-search').val();
    $('.js-shopping-list-search').val('');
    renderShoppingList(searchEntry);

  });
}

function handleRename(){
  $('.shopping-list').on('click', '.shopping-item', event => {
    const newName = prompt('Change item name', 'New name');
    const id = getItemIdFromElement(event.currentTarget);
    updateNameforListItem(id, newName);
    renderShoppingList();
  });
}
